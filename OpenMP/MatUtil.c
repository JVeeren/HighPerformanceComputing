...
void ST_APSP(int *mat, const size_t N) //seq
{
	for (int k = 0; k < N; k++)
		for (int i = 0; i < N; i++)
			for (int j = 0; j < N; j++)
...
void MT_APSP(int *mat, const size_t N) //parallel
{
...
#pragma omp parallel for private(i, j) schedule(static) // Each thread has its own variable i and j and share the matrix (mat)
		for (i = 0; i < N; i++)
		{
